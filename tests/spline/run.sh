#!/bin/sh
# tests/spline/run.sh — spline/Bezier tessellation (links libhobbycad)
# SPDX-License-Identifier: GPL-3.0-only
set -e
ROOT=$(cd "$(dirname "$0")/../.." && pwd)
BUILD=${1:-$ROOT/build}
LIB=$BUILD/src/libhobbycad/libhobbycad.a
[ -f "$LIB" ] || { echo "error: $LIB not found; build first" >&2; exit 2; }
...
if [ "$STATUS" -eq 0 ]; then echo "spline tests: ALL PASS"; else echo "spline tests: FAILURES"; fi
exit "$STATUS"
