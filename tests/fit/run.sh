#!/bin/sh
# =====================================================================
#  tests/fit/run.sh — 2D B-spline least-squares fit (OCCT-backed)
#  SPDX-License-Identifier: GPL-3.0-only
# =====================================================================
#  Dependency-free, same approach as tests/dxf/run.sh: link the built
#  static library directly. Build the project first.
set -e
...
for src in "$(dirname "$0")"/*.cpp; do
    ...
    g++ -std=c++17 -I"$ROOT/src/libhobbycad" -I"$OCC_INC" $QT_CFLAGS \
        "$src" "$LIB" $QT_LIBS $HC_IMG_LIBS $OCC_LIBS -lslvs -o "$OUT/$name" || { STATUS=1; continue; }
    "$OUT/$name" || STATUS=1
done
if [ "$STATUS" -eq 0 ]; then echo "fit tests: ALL PASS"; else echo "fit tests: FAILURES"; fi
exit "$STATUS"
