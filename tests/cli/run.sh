#!/bin/sh
# =====================================================================
#  tests/cli/run.sh — CLI engine tests
...
#
#  CliEngine is not a QObject, so no moc output is needed here.
#
#  Usage:  tests/cli/run.sh [path/to/build]
set -e
...
for src in "$(dirname "$0")"/*.cpp; do
    ...
    g++ -std=c++17 -fPIC \
        -I"$ROOT/src/hobbycad/cli" -I"$ROOT/src/hobbycad" -I"$ROOT/src/libhobbycad" \
        -I"$OCC_INC" $QT_CFLAGS "$src" \
        "$ROOT/src/hobbycad/cli/cliengine.cpp" \
        "$ROOT/src/hobbycad/cli/clihistory.cpp" \
        "$LIB" $QT_LIBS $HC_IMG_LIBS $OCC_LIBS $SLVS_LIB -o "$OUT/$name" || { STATUS=1; continue; }
    echo "-- $name"
    "$OUT/$name" || STATUS=1
done

if [ $STATUS -eq 0 ]; then
    echo "cli tests: ALL PASS"
else
    echo "cli tests: FAILURES"
fi
exit $STATUS
